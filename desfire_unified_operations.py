#!/usr/bin/env python3
"""
DESFire EV1 Unified Operations
==============================
...
Autor: Basado en análisis de scripts existentes
Fecha: 2025
"""
...
class DESFireOperations:
    ...
    def send_command(self, command):
        ...
        if len(command) == 1:
            ...
            cmd_byte = command[0]
            data = command[1:]
            # Para comandos largos (>255 bytes), usar formato extendido
            if len(data) > 255:
                apdu = [0x90, cmd_byte, 0x00, 0x00, 0x00, (len(data) >> 8) & 0xFF, len(data) & 0xFF] + list(data)
            else:
    ...
    def authenticate_des(self, key_number, key):
        ...
        try:
            ...
            expected_rnd_a = rnd_a[1:] + rnd_a[:1]
            if decrypted_rnd_a != expected_rnd_a:
                self.log("❌ Verificación RndA falló")
                return False
    ...
    def authenticate_auto(self, key_number, aes_key=None, des_key=None):
        ...
        self.log("❌ Todas las autenticaciones fallaron")
        return False
    ...
    def get_file_ids(self):
        ...
        if not self.authenticated:
            self.log("❌ Debe autenticarse primero")
            return []
    ...
    def get_file_info(self, file_id):
        ...
        if response[0] == 0x00:
            ...
            if len(settings) >= 7:
                ...
                self.log(f"❌ Respuesta inválida: {len(settings)} bytes")
                return None
        else:
    ...
    def delete_file(self, file_id):
        ...
        self.log(f"🗑️ Eliminando archivo {file_id}")
    ...
    def write_file_data(self, file_id, offset, data):
        ...
        self.log(f"✍️ Escribiendo {len(data)} bytes en archivo {file_id} (offset {offset})")
    ...
    def read_file_data(self, file_id, offset=0, length=None, auto_trim=True):
        ...
        if response[0] == 0x00:
            ...
            if auto_trim and length is not None and len(file_data) > length:
                ...
                self.log(f"✂️ Recortando: {len(expected_data)} bytes útiles, {len(extra_data)} extras")
...
def demo_basic_operations():
    ...
    try:
        ...
        if not desfire.select_application(test_aid):
            print(f"⚠️ Aplicación {test_aid.hex().upper()} no encontrada")
            return False
        ...
        test_data = b"Hola DESFire EV1! Operaciones unificadas funcionando correctamente."
        print(f"\n✍️ Escribiendo datos de prueba...")
